var CertificationIsSubmitted = React.createClass({
  ...
  hasFinancials() {
      if ( this.state.certification.operating_expenses && this.state.certification.file_budget ) {
        return "true"
      } else if ( this.state.certification.operating_expenses || this.state.certification.file_budget ) {
        return "progress"
      } else {
        return "false"
      }
  ...
  hasPayments() {
    if ( this.state.artist_payments || this.state.certification.qb_pl) {
      if (this.state.artist_payments.length > 1 || this.state.certification.qb_pl) {
        return 'true'
  ...
  formatDates() {
    if (moment(this.state.certification.fiscal_start).format('Y') == moment(this.state.certification.fiscal_end).format('Y') ) {
      var formatted_date = moment(this.state.certification.fiscal_start).format('MMM D') + " - " + moment(this.state.certification.fiscal_end).format('MMM D, YYYY');
    } else {
      var formatted_date = moment(this.state.certification.fiscal_start).format('MMM D, YYYY') + " - " + moment(this.state.certification.fiscal_end).format('MMM D, YYYY');
    }
  ...
  var next = <button className="btn next" onClick={this.getNext}>Next</button>
   if (this.state.contentState == 0) {
    ...
    if (this.state.certification.operating_expenses) {
      ...
      operating_expenses = <h5><a onClick={this.viewFinancials}>Enter your operating expenses</a> to personalize.</h5>
    }
    var contentState = <div className="fee-schedule">
      <div className="intro">
        <h1><span>My Fee Schedule</span></h1>
        <span><h5>Fiscal Year: {this.formatDates()}</h5>
  ...
  viewFinancials() {
    ...
    $('.status .item').removeClass('active')
    $('.status .item[data-id="1"]').addClass('active')
  },
  feeSchedule() {
    var btn
    if (this.state.certification.operating_expenses != null) {
      btn = <button className="btn fee-schedule" onClick={this.viewFeeSchedule}>My Fee Schedule</button>
    } else {
      btn = <button className="btn fee-schedule" onClick={this.viewFeeSchedule}>Fee Schedule</button>
    }
    return btn
  },
  render() {
